...
public class TeamASelectionManager : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if(Input.GetKeyDown(KeyCode.Space)) {
            ...
            //myTeam (team A) finalization
            for(int i = 0; i < 11; i++) {
                //finalize batting line ups
                FIrstInningsManager.teamAPlayerNames[i] = theTeam19[i].GetComponent<Text>().text;
                FirstInningsManagerB.teamAPlayerNames[i] = theTeam19[i].GetComponent<Text>().text;
                SecondInningsManager.teamAPlayerNames[i] = theTeam19[i].GetComponent<Text>().text;
                SecondInningsManagerB.teamAPlayerNames[i] = theTeam19[i].GetComponent<Text>().text;

                //finalize batting and bowling tiers
                //team A batting
                FIrstInningsManager.battingTeamTiers[i] = int.Parse(theTeam19BattingTiers[i].GetComponent<Text>().text);
                SecondInningsManagerB.battingTeamTiers[i] = int.Parse(theTeam19BattingTiers[i].GetComponent<Text>().text);
                //team A bowling
                SecondInningsManager.bowlingTeamTiers[i] = int.Parse(theTeam19BowlingTiers[i].GetComponent<Text>().text);
                FirstInningsManagerB.bowlingTeamTiers[i] = int.Parse(theTeam19BowlingTiers[i].GetComponent<Text>().text);

                //my team average batting strength
            ...
            //opposition (team B) finalization
            for(int i = 0; i < 11; i++) {
                //finalize batting line ups
                FIrstInningsManager.teamBPlayerNames[i] = opposition19PlayerNames[i];
                FirstInningsManagerB.teamBPlayerNames[i] = opposition19PlayerNames[i];
                SecondInningsManager.teamBPlayerNames[i] = opposition19PlayerNames[i];
                SecondInningsManagerB.teamBPlayerNames[i] = opposition19PlayerNames[i];

                //finalize batting and bowling tiers
                //team B bowling
                FIrstInningsManager.bowlingTeamTiers[i] = opposition19BowlingTiersInt[i];
                SecondInningsManagerB.bowlingTeamTiers[i] = opposition19BowlingTiersInt[i];
                //team B batting
                SecondInningsManager.battingTeamTiers[i] = opposition19BattingTiersInt[i];
                FirstInningsManagerB.battingTeamTiers[i] = opposition19BattingTiersInt[i];
