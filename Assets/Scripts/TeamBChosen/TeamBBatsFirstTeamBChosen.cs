...
public class TeamBBatsFirstTeamBChosen : MonoBehaviour
{
    //teamA -> opposition -> bowlingTeam
    //teamB -> myTeam -> battingTeam
    ...
    [SerializeField] GameObject playerInstructionText;

    bool clickAllowed, skipOut, skipNotOut, pickedBowler, battingbattingTeamowledOut, inningsComplete;
    void Awake()
    {
        battingTeamTotal = 0;
        totalWickets = 0;
    ...
    void Start()
    {
        ...
        playerInstructionText.GetComponent<Text>().text = "Start Innings (Press Spacebar)";
    ...
    {
        if(inningsComplete) {
            playerInstructionText.GetComponent<Text>().text = "Continue (Press Spacebar)";
        }
        if(Input.GetKeyDown(KeyCode.Space) && !inningsComplete && clickAllowed) {
            clickAllowed = false;
            pickedBowler = false;
           if(totalOversBowled == 0) {
               playerInstructionText.GetComponent<Text>().text = "Start Next Over (Press Spacebar)";
        ...
        if(Input.GetKeyDown(KeyCode.Space) && inningsComplete) {
            if(deliveriesThisOver == 6) {
                ...
                InningsBreakManager.nextSceneName = "TeamAChasingTeamBChosen";
